...
for iter=1:totalIter
    ...
    for iset = 1:length(imageSets)
        ...
        %parfor i=1:length(currentData)
            ...
            idx = getDistIdx([tempData.x tempData.y], [x y]); % see the poster, all the sift points are assigned to the nearest 2by2 points
            %so all the sift points that have the same nearest 2by2 points
            %are put together
            overallIdx = cell(length(x), 1);  % 4
            for j=1:length(x)
                overallIdx{j} = find(idx==j);
            end
           
           
            nonzero_idx = cellfun(@(x) ~isempty(x),overallIdx); % 1,2,3,4
            ...
            integralData{i} = sparse(length(x), dictionarySize);  % max pooling ?
