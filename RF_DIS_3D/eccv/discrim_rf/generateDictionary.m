...
imageIdx = randi(length(data.tr), numImages, 1);  % randomly sample images, we only need numimages to build the dictionary 
patchIdx = zeros(numImages, 1);
siftCount = zeros(numImages, 1);
flipIdx = ones(numImages, 1);
if(config.includeFlippedImages)
    flipIdx = randi(2, numImages, 1); % randomly choose flip or not flip images
end
...
for i=1:numImages
  ...
  patchIdx(i) = randi(length(tempData.patchSizes), 1); % randomly choose the patch, because we have five patches, remember the patch index
  siftCount(i) = length(tempData.data{patchIdx(i)});   % remember how many sift describers in the image with this patch
end
