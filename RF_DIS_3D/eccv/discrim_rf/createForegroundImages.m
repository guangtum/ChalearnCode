...
maxDimension = 300;
sprintf('test2')
sprintf(VOCopts.action.imgsetpath, imgset)  %VOCopts.action.imgsetpath=[VOCopts.datadir VOCopts.dataset '/ImageSets/Action/%s.txt']     2010_006097
                                            %2010_006100   2010_006102   ;2010_006104
...
for i=startIdx:endIdx
    imageFName = [input_folder ids{i} '.jpg'];   % ids{i}--> 2010_006100
    disp(['Image ' num2str(i) ' of ' num2str(numIdx)])
    
    rec=PASreadrecord(sprintf(VOCopts.annopath,ids{i})); % annotation for the image
    
    img_wid = rec.imgsize(1);
    img_hgt = rec.imgsize(2);
    img = imread(imageFName);
    
    for j=1:length(rec.objects)  % hi_fg ='randomforest/llc_extraction/dataset_fg';% path where training and test images exist
        %outFName = [output_folder ids{i} '_' num2str(j) '.jpg']; % output_folder = 'dataset_fg/';
        outFName = [ids{i} '_' num2str(j) '.jpg'];  % some figures have more than one targets (e.g. people)
