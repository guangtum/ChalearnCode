...
% file path   outputFile = fullfile(config.outputFolder, [data.name '/' imageSet flip_str '.mat']);
...
for i = 1 : length(action_name)
    ...
    % filegt=trainingfilegt='randomforest/llc_extraction/VOC2011ori/TrainVal/VOCdevkit/VOC2011/ImageSets/Action/'; % the path where the list of each action training data exists
    % imageset = 'trainval'
    % get the id of the images that belongs to each action class
    disp(action_name{i})
    ...
    for j = 1 : length(currids)
        ...
        % do this for only trainval , not for test
     
        if currgt(j) ~= 1 % 1 means this image belongs to class i
            continue;
        end
       
        
        count = 0;
        for k = 1 : image_num   % image_num includes all of the train and val images
                                % but currids has more numbers
            if (strmatch(currids{j},fg.ids{k}) == 1) % ids is found in fg.ids
               
                %2011_006503  1 -1  one option
                %2011_006503  2 -1
                %2011_006503  3 -1
                
                %2011_006503  1 1  another option
                %2011_006503  2 1
                %2011_006503  3 1  --> gt{k} = [i i i]
                
                %2011_006503  1 1  another option    ?????
                %2011_006503  2 -1
                %2011_006503  3 1  --> gt{k} = [i]
                
                %2011_006503  1 -1  another option   ?????
                %2011_006503  2 1
                %2011_006503  3 1  --> gt{k} = [i]   
                
                % my understanding is here only choose  2011_006503  1 1
                % this case, 1, 1, if they can not find a match for fg.ids{k}, the gt is
                % empty
                count = count + 1;
                if count == currobjId(j)  % here k is the index of image in the training sets
                    gt{k} = [gt{k}, i];   % here i instead of action class 
                end                       % gt{k} can have more than two classes, which means the k images have more that two actions in one figures 
            end
        end
    end
end

class_num = length(action_name) + 1;  % plus one means others? a special class
for i = 1 : length(gt)
    if isempty(gt{i})
        gt{i} = class_num;   % this means the training image does not include a predefined action class 
    end
end
...
for i = 1 : image_num
	...
	if divide2 == 0
	    % fg data
	   	width = numel(unique(fg.imageInfo{i}.y)); % to get the unique number
	    height = numel(unique(fg.imageInfo{i}.x));
	    % note that the width and height are inversed
	    fwrite(fid, int32(width), 'int');  % output width of the image   % we have width(is a number) choise to choose a image width.
    	fwrite(fid, int32(height), 'int'); % output height of the image
    
	    % starting locations of the LLC results, here we get a index4 number of llc histograms (for each position, the histogram is the max pooling of different pathces)
	    fg.integralData{i} = full(fg.integralData{i}); % compare to sparse
	    start_id = 0;
	    fwrite(fid, int32(start_id), 'int');
	    for j = 1 : size(fg.integralData{i}, 1) % how many llc for each image, read each llc feature
	        start_id = start_id + numel(find(fg.integralData{i}(j, :) ~= 0)); % we need to know how many feature values are not zero
	        fwrite(fid, int32(start_id), 'int');
	    end
        ...
        % ???????guess
        % code1 value1 value2 value3 value4 value5 value6 value7 value8
        % code2 value1 value2 value3 value4 value5 value6 value7 value8
        %
	    fwrite(fid, int32(codeIndex), 'int');  % is a vector [1 2 3 4 5 6 7 8 .... 1024]
	    fwrite(fid, single(LLCValue), 'single'); % is a matrix  single????
    ...
    for j = 1 : numel(bg.ids)   
        if (strmatch(bg.ids{j},fg.ids{i}) == 1)
            bgID = j;  % for each foreground image, find the matched background image
        end
    end
    assert(bgID ~= 0);
