...
int GetTreeNum()
{
  int treeNum = 0;
  char filename[1024];
  sprintf(filename, "trees/tree_%d%d%d.txt", treeNum / 100, (treeNum % 100) / 10, treeNum % 10);
...
bool InputTree(TREENODE *treeNodes, int treeID, int maxNodeNum)
{
  // check if the tree filename exists
  char filename[1024];
  sprintf(filename, "trees/tree_%d%d%d.txt", treeID / 100, (treeID % 100) / 10, treeID % 10);
...
void ReadTrees(vector<TREENODE*> &trees, TREENODE *treeMemory, int treeNum, PARAMETER param)
{
  ...
  {
    if (InputTree(treeMemPTR, i, maxNodeNum))  // the tree to be read exists and is a valid tree
    {
