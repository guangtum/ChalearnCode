function annot_model_segmentdata(config)
...
% models
   switch config.overallmodel,
       case 'Train' % we train the model using the training dataset 
           ...
           annotdatapath = fullfile(traindatapath,matfilepath);   % 
           % create the path to save the model
           modelsegpath = fullfile(config.seg.inittrainedpath,['Train_Segmentationmodel_', matfilepath, '.mat']);
           saveannotations = fullfile(config.seg.inittrainedpath,['Annot_Segmentationmodel_', matfilepath, '.mat']);
           %train_singlesample_segsvm(fulltraindatapath,modelsavepathname,config);
           if exist(saveannotations,'file')~=2
               anno_samples_basedonsegmodel(annotdatapath,modelsegpath,saveannotations,fi,config);
           end
