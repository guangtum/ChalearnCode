...
%   tstart_batch = tic;
    ...
    act_l1_list = transactConvISA(data_batch, isa1, isa2, postact.layer1);  % the first layer   2400*(number of data)
    
    act_out = activateISA(act_l1_list, isa2, postact.layer2);  % the second layer    % 100*(number of data)
    ...
    if isfield(postact.layer2, 'gpu')
        ...
        act_l1_pca_reduced(:,nr_done+1:nr_done+nr_to_calc) = isa2.V(1:size(isa2.H,1),:)*act_l1_list; % V comes from PCA,  (200(1:100)*2400)*(2400*(number of data)) =100*(number of data)
    end
