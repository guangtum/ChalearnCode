function [K_train, K_test] = compute_kernel_matrices(M_train, M_test, kernelty, mode)
...
if strcmp(mode, 'both')||strcmp(mode, 'train')
    ...
    K_train = exp(-slmetric_pw(M_train', M_train', kernelty)/A);  %chisq  intersectdis
...
if strcmp(mode, 'both')||strcmp(mode, 'test')
    ...
    K_test = exp(-slmetric_pw(M_test', M_train', kernelty)/A);  %chisq
